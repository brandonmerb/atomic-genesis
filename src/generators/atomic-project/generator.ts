...
import { AtomicProjectGeneratorSchema } from './schema';
...
function normalizeOptions(tree: Tree, options: AtomicProjectGeneratorSchema): NormalizedSchema {
  let name = names(options.projectName).fileName
  let organizationName;
  if (name.includes("@") && name.split('/').length > 1) {
    organizationName = options.projectName.split('/')[0];
  }
  let projectName = !!organizationName ? options.projectName.split('/')[1] : name;
...
function addFiles(tree: Tree, options: NormalizedSchema) {
    const templateOptions = {
      ...
      packageName: !!options.organizationName ? `${options.organizationName}/${options.projectName}` : options.projectName,
      moduleName: options.projectName
    };
